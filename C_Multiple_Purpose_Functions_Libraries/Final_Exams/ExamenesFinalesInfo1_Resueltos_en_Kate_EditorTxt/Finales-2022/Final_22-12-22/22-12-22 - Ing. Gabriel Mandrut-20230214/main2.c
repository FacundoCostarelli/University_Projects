...
int pos,i;
	...
	if (pb->cant==0)	//buffer vacio
		return 0;

	// buscamos si hay un elementop para el destino indicado.
	pos= buscar (pb, dest);
	if (pos<0)
		return 0;// ningun elemento corresponde al destino indicado

	*pdat=pb->p[pos].st;

	//--- eliminamos el destino del elemento;
	del_dest(&pb->p[pos].dest,dest);

	if (!pb->p[pos].dest)	// elemento sin destinos
	{
		// hay que eliminar el elemento 
		for (i=pos;i<pb->cant;i++)
			pb->p[i]=pb->p[i+1];
		pb->cant--;

		
	// si ya se eliminaron muchos datos. se puede hacer un redimensionamiento del bloque.	
...
// liberación del bloque de memoria
...
uint32_t xdest[]={ 	26,46,64321,1,456,61 };	
	...
	liberar (&buff);	// no haria falta ya que la lista se vacio antes
