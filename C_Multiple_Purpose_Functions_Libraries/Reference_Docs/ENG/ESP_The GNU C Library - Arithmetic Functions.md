# Original Text - The GNU C Library - Arithmetic Functions.pdf

Go to the ﬁrst, previous , next, last section, table of contents .
...
the header ﬁles `math.h' and `complex.h' .
Integers
...
with diﬀerent inherent data sizes (word sizes), so each type may have diﬀerent
ranges on diﬀerent machines. The problem with this is that a program often needs
...
rounds towards zero. But in other C implementations, `/' may round diﬀerently
...
These functions are speciﬁed to return a result r such that the value
r.quot*denominator  + r.rem equals numerator .
To use these facilities, you should include the header ﬁle `stdlib.h' in your program.
Data Type:div_t
...
Most computer hardware has support for two diﬀerent kinds of numbers: integers
...
exponent, and @math{M} the mantissa. See section Floating Point RepresentationConcepts , for details. (It is possible to have a diﬀerent base for the exponent, but
all modern hardware uses @math{2}.)
Floating-point numbers can represent a ﬁnite subset of the real numbers. While
...
when the results of a calculation were inaccurate. Modern computers implement
...
Floating-Point Number Classiﬁcation Functions
ISO C99 deﬁnes macros that let you determine what sort of ﬂoating-point number a
variable holds.
Macro: int fpclassify (ﬂoat-type x)
This is a generic macro which works on all ﬂoating-point types and which
...
FP_NAN
The ﬂoating-point number x is "Not a Number" (see section Inﬁnity and
NaN)
FP_INFINITE
The value of x is either plus or minus inﬁnity (see section Inﬁnity and
NaN)
FP_ZERO
The value of x is zero. In ﬂoating-point formats like IEEE 754, where zero
...
This macro returns a nonzero value if x is ﬁnite and normalized. It is
equivalent to
...
This function returns -1 if x represents negative inﬁnity, 1 if x represents
...
otherwise.
Note: The isnan macro deﬁned by ISO C99 overrides the BSD function. This is
...
(isnan) (x)
Function: int ﬁnite (double x)
Function: int ﬁnitef (ﬂoat x)
Function: int ﬁnitel (long double x)
This function returns a nonzero value if x is ﬁnite or a "not a number" value,
and zero otherwise.
...
The GNU library does not do likewise, because that does not ﬁt the ISO C
speciﬁcation.
Portability Note: The functions listed in this section are BSD extensions.
Errors in Floating-Point Calculations
FP Exceptions
The IEEE 754 standard deﬁnes ﬁve exceptions that can occur during a
...
in the ﬂoating-point status word, and the program continues as if nothing had
happened. The operation produces a default value, which depends on the exception
(see the table below). Your program can check the status word to ﬁnd out which
exceptions happened.
...
change the eﬀect of the signal.
...
`Division by Zero'
This exception is raised when a ﬁnite nonzero number is divided by zero. If no
trap occurs the result is either @math{+@inﬁnity{}} or
@math{-@inﬁnity{}}, depending on the signs of the operands.
`Overflow'
This exception is raised whenever the result cannot be represented as a ﬁnite
...
representable ﬁnite number with the sign of the intermediate result.
3. Round toward @math{-@inﬁnity{}} carries positive overﬂows to the
largest representable ﬁnite number and negative overﬂows to
@math{-@inﬁnity{}}.
...
This exception is signalled if a rounded result is not exact (such as when
calculating the square root of two) or a result overﬂows without an overﬂow
trap.
Inﬁnity and NaN
...
of calculations that produce inﬁnity or NaN:
@ifnottex
@math{1/0 = @infinity{}}
@math{log (0) = -@infinity{}}
@math{sqrt (-1) = NaN}
...
sensible output. Inﬁnities propagate through calculations as one would expect: for
...
NaN, and negative inﬁnity is smaller than all values except itself and NaN. NaN is
...
section Floating-Point Number Classiﬁcation Functions ). In addition, <, >, <=, and >=
...
Macro: ﬂoat INFINITY
An expression representing positive inﬁnity. It is equal to the value produced
by mathematical operations like 1.0 / 0.0. -INFINITY represents negative inﬁnity.
...
produced when you divide a positive number by negative inﬁnity, or when a
negative result is smaller than the limits of representation. Negative zero behaves
...
the diﬀerent exceptions. Each constant is deﬁned if and only if the FPU you are
...
These functions allow you to clear exception ﬂags, test for exceptions, and save
...
equivalent to `(status & excepts)' . The actual implementation may be very diﬀerent,
of course.
...
should clear all the ﬂags ﬁrst. Here is a simple example of the way to use
fetestexcept :
{
  double f;
  int raised;
...
domain errors, overﬂows, and underﬂows, respectively. Math functions do
several things when one of these errors occurs. In this manual we will refer to the
complete response as signalling a domain error, overﬂow, or underﬂow.
...
rounded to ﬁt into the destination type. This ensures that results are as precise as
...
numeric stability: round-oﬀ errors in a lengthy calculation will remain smaller
than half of FLT_EPSILON .
Round toward plus Inﬁnity.
All results are rounded to the smallest representable value which is greater
than the result.
Round toward minus Inﬁnity.
...
At any time one of the above four rounding modes is selected. You can ﬁnd out
...
operation; also, some hardware requires you to compile your program diﬀerently
for it to work. The resulting code may run slower. See your compiler
documentation for details.
Floating-Point Control Functions
IEEE 754 ﬂoating-point implementations allow the programmer to decide whether
...
Handling .
Note: IEEE 754 says that trap handlers are given details of the exceptionalThe GNU C Library - Arithmetic Functions https://ftp.gnu.org/old-gnu/Manuals/glibc-2.2.3/html_...
...
the exception ﬂags, the set of exceptions that generate traps, and the rounding
mode. This information is known as the ﬂoating-point environment.
The functions to save and restore the ﬂoating-point environment all use a variable
...
Then clear all exception ﬂags, and set the FPU to trap no exceptions. Not all
...
or feholdexcept .
• The special macro FE_DFL_ENV which represents the ﬂoating-point environment
...
To set the ﬂoating-point environment, you can use either of these functions:
...
parameter except . The individual excepetions are described in section
Examining the FPU status word . Only the speciﬁed exceptions are enabled,
...
parameter except . The individual excepetions are described in section
Examining the FPU status word . Only the speciﬁed exceptions are disabled,
the status of the other exceptions is not changed.
The function returns the previous enabled exceptions in case the operation
was successful, -1 otherwise.
...
Arithmetic Functions
The C library provides functions to do basic operations on ﬂoating-point numbers.
...
a number. The absolute value of a real number x is x if x is positive, - x if x is
...
eﬀiciently perform certain low-level manipulations on ﬂoating point numbers that
are represented internally using a binary radix; see section Floating PointRepresentation Concepts . These functions are required to have equivalent behavior
even if the representation does not use a radix of 2, but of course they are unlikely
to be particularly eﬀicient in those cases.
...
and a very small number if it does not. If x is inﬁnity, the return value is
inﬁnity.
...
significand  returns the mantissa of x scaled to the range @math{[1, 2)}. It is
...
ﬂoating-point values. Some of these functions convert ﬂoating point numbers to
integer values. They are all declared in `math.h'.
You can also convert ﬂoating-point numbers to integers simply by casting them to
int. This discards the fractional part, eﬀectively rounding towards zero. However,
...
ﬂoating-point format). Thus, trunc (1.5)  is 1.0 and trunc (-1.5)  is -1.0.
...
integer; some machines support other modes, but round-to-nearest is always
...
point numbers. Each is a little diﬀerent; pick the one that suits your problem.
...
denominator . Speciﬁcally, the return value is numerator  - n * denominator , where n
is the quotient of numerator  divided by denominator , rounded towards zero to
an integer. Thus, fmod (6.5, 2.3)  returns 1.9, which is 6.5 minus 4.6.
...
These functions are like fmod except that they rounds the internal quotient n toThe GNU C Library - Arithmetic Functions https://ftp.gnu.org/old-gnu/Manuals/glibc-2.2.3/html_...
...
of the denominator . The diﬀerence between fmod (numerator , denominator ) and drem
...
support it) and this is one of the few operations that can tell the diﬀerence.
...
the least signiﬁcant bit in the mantissa is added or subtracted, depending on
...
The standard C comparison operators provoke exceptions when one or other of the
...
not raise exceptions when NaN is examined. All of the functions are implemented
...
Macro: int isgreaterequal (real-ﬂoating x, real-ﬂoating y)
...
Macro: int isless (real-ﬂoating x, real-ﬂoating y)
...
are NaN.
Macro: int isunordered (real-ﬂoating x, real-ﬂoating y)
...
Note: There are no macros isequal or isunequal. They are unnecessary, because the
...
The fdim function returns the positive diﬀerence between x and y. The positive
...
preferable. However, it causes problems if you want to use the identiﬁer I for
...
Function: ﬂoat cimagf (complex ﬂoat z)
...
Function: ﬂoat cargf (complex ﬂoat z)
...
sphere. Values with a inﬁnite imaginary part are projected to positive inﬁnity
on the real axis, even if the real part is NaN. If the real part is inﬁnite, the
result is equivalent to
INFINITY + I * copysign (0.0, cimag (z))
Parsing of Numbers
This section describes functions for "reading" integer and ﬂoating-point numbers
...
more robust by ﬁnding the tokens in the string by hand, then converting the
numbers one by one.
...
of the functions in this section. It is seemingly useless but the ISO C standard uses
...
errno  before the call and check it afterward, in case there was overﬂow.
...
Function: unsigned long int strtoul (const char *retrict string, char **restrictThe GNU C Library - Arithmetic Functions https://ftp.gnu.org/old-gnu/Manuals/glibc-2.2.3/html_...
...
If string  depicts a negative number, strtoul acts the same as strtol  but casts
...
The atoll function was introduced in ISO C99. It too is obsolete (despite
having just been added); use strtoll instead.
All the functions mentioned in this section so far do not handle alternative
representations of characters as described in the locale data. Some locales specify
...
are whitespace is determined by the isspace function (see sectionClassiﬁcation of Characters ). These are discarded.
• An optional plus or minus sign ( `+' or `-').
• A ﬂoating point number in decimal or hexadecimal format. The decimal
format is:
...
ﬂoating-point format doesn't support inﬁnities. You can prepend a "+" or "-" to
...
The wcstod, wcstof, and wcstol functions are equivalent in nearly all aspect to the
...
This function is similar to the strtod function, except that it need not detect
...
will be rounded to one signiﬁcant digit.
If ndigit  decimal digits would exceed the precision of a double it is reduced to a
system-speciﬁc value.
...
This function is equivalent to ecvt except that it takes a long double  for the ﬁrst
...
This function is equivalent to fcvt except that it takes a long double  for the ﬁrst
parameter and that ndigit  is restricted by the precision of a long double .
Function: char * qgcvt (long double value, int ndigit, char *buf)
This function is equivalent to gcvt except that it takes a long double  for the ﬁrst
parameter and that ndigit  is restricted by the precision of a long double .
The ecvt and fcvt functions, and their long double  equivalents, all return a string
located in a static buﬀer which is overwritten by the next call to the function. The
GNU C library provides another set of extended functions which write the
converted string into a user-supplied buﬀer. These have the conventional _r suﬀix.
gcvt_r is not necessary, because gcvt already uses a user-supplied buﬀer.
...
user-speciﬁed buﬀer pointed to by buf, with length len.
This function is a GNU extension.
Go to the ﬁrst, previous , next, last section, table of contents .The GNU C Library - Arithmetic Functions https://ftp.gnu.org/old-gnu/Manuals/glibc-2.2.3/html_...
